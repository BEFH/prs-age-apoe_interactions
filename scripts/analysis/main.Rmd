...
pacman::p_load(dplyr, readr, tidyr, magrittr, broom, forcats, purrr, car,
               ...
               rstatix, ggstance, knitr, here, stringr, gtsummary)
...
printmod <- function (...) {
  #fs <- function(x) ifelse(x < 0.001, scientific(x, 1), sprintf("%.3f", x))
  if (...length() == 2) {
    t1 = tbl_regression(..1, pvalue_fun = fs)
    t2 = tbl_regression(..2, pvalue_fun = fs)
    tbl_merge(
      tbls = list(t1, t2),
      tab_spanner = c("**Without interactions**", "**With interactions**")
    ) %>% wordcompat
  } else {
    tbl_regression(..1, pvalue_fun = fs) %>% wordcompat
  }
}
...
prettyvar <- . %>%
  rename(age = aaoaae2) %>%
  rename_all(funs(str_replace(., "jointPC", "PC")))
scores %<>% prettyvar
...
status_glm_sa <- glm(all_status10 ~ age * PRS_1e_minus05 * E4strat + sex +
  PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = age_strat)
